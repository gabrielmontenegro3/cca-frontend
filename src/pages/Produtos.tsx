...
import { dadosEstaticos, getFornecedorById, getLocalById, getGarantiaById } from '../data/dadosEstaticos'
...
const Produtos = ({ setActivePage }: ProdutosProps) => {
  const [produtosFiltrados, setProdutosFiltrados] = useState(dadosEstaticos.produtos)
  const [busca, setBusca] = useState('')
  const [showViewModal, setShowViewModal] = useState(false)
  const [viewingProduto, setViewingProduto] = useState<typeof dadosEstaticos.produtos[0] | null>(null)
  ...
  useEffect(() => {
    ...
    if (produtoIdToView) {
      ...
      const produto = dadosEstaticos.produtos.find(p => p.id === id)
      if (produto) {
  ...
  useEffect(() => {
    if (!busca.trim()) {
      setProdutosFiltrados(dadosEstaticos.produtos)
    } else {
      const termoBusca = busca.toLowerCase()
      const filtrados = dadosEstaticos.produtos.filter((produto) => {
  ...
  const abrirModalVisualizar = (produto: typeof dadosEstaticos.produtos[0]) => {
  ...
  return (
    <div className="space-y-6">
            ...
            <tbody className="divide-y divide-gray-700/50">
              {produtosFiltrados.length === 0 ? (
                ...
                produtosFiltrados.map((produto) => {
                  const fornecedor = getFornecedorById(produto.fornecedor_id)
                  const locais = produto.locais_ids.map(id => getLocalById(id)).filter(Boolean)
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {fornecedor ? (
                          <button
                            ...
                            className="text-sm text-gray-300 hover:text-blue-400 transition-colors flex items-center gap-2 text-left"
                          >
                            ...
                            <div className="flex flex-wrap gap-1.5">
                              {locais.map((local) => local && (
                                <button
                                  ...
                                  className="px-2.5 py-1 bg-gray-700/50 rounded-lg text-xs border border-gray-600 hover:bg-gray-600 hover:border-gray-500 transition-colors"
                                >
                                  {local.nome}
                                </button>
                              ))}
      ...
      {showViewModal && viewingProduto && (() => {
        const fornecedor = getFornecedorById(viewingProduto.fornecedor_id)
        const garantia = getGarantiaById(viewingProduto.garantia_id)
        const locais = viewingProduto.locais_ids.map(id => getLocalById(id)).filter(Boolean)
                        ...
                        {garantia.perda_garantia.length > 0 && (
                          <div className="border-t border-gray-600/50 pt-4 mt-4">
                            ...
                            <ul className="mt-2 space-y-1">
                              {garantia.perda_garantia.map((item, index) => (
