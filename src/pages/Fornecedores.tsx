...
import { dadosEstaticos, getGarantiaById } from '../data/dadosEstaticos'
...
const Fornecedores = ({ setActivePage }: FornecedoresProps) => {
  const [fornecedoresFiltrados, setFornecedoresFiltrados] = useState(dadosEstaticos.fornecedores)
  const [busca, setBusca] = useState('')
  const [showViewModal, setShowViewModal] = useState(false)
  const [viewingFornecedor, setViewingFornecedor] = useState<typeof dadosEstaticos.fornecedores[0] | null>(null)
  ...
  useEffect(() => {
    ...
    if (fornecedorIdToView) {
      localStorage.removeItem('fornecedorToView')
      const id = parseInt(fornecedorIdToView)
      const fornecedor = dadosEstaticos.fornecedores.find(f => f.id === id)
      if (fornecedor) {
  ...
  useEffect(() => {
    if (!busca.trim()) {
      setFornecedoresFiltrados(dadosEstaticos.fornecedores)
    } else {
      const termoBusca = busca.toLowerCase()
      const filtrados = dadosEstaticos.fornecedores.filter(
        (fornecedor) =>
          fornecedor.nome?.toLowerCase().includes(termoBusca) ||
          fornecedor.cnpj?.toLowerCase().includes(termoBusca) ||
          fornecedor.telefone1?.toLowerCase().includes(termoBusca) ||
          fornecedor.telefone2?.toLowerCase().includes(termoBusca) ||
          fornecedor.endereco?.toLowerCase().includes(termoBusca) ||
          fornecedor.local?.toLowerCase().includes(termoBusca)
      )
      setFornecedoresFiltrados(filtrados)
    }
  }, [busca])

  const abrirModalVisualizar = (fornecedor: typeof dadosEstaticos.fornecedores[0]) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-5 border border-gray-700 shadow-xl">
        <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center">
          <div className="flex-1 relative">
            <input
              type="text"
              placeholder="Buscar por nome, CNPJ, telefone, endereço, localização..."
              value={busca}
                      ...
                      <div className="flex flex-col gap-1">
                        {fornecedor.telefone1 && (
                          <div className="text-sm text-gray-300 flex items-center gap-2">
                            ...
                            <span>{fornecedor.telefone1}</span>
                          </div>
                        )}
                        {fornecedor.telefone2 && (
                          <div className="text-sm text-gray-300 flex items-center gap-2">
                            ...
                            <span>{fornecedor.telefone2}</span>
                          </div>
                        )}
                        {!fornecedor.telefone1 && !fornecedor.telefone2 && (
      ...
      {showViewModal && viewingFornecedor && (() => {
        const produtosFornecedor = dadosEstaticos.produtos.filter(p => p.fornecedor_id === viewingFornecedor.id)
                          ...
                          <table className="w-full">
                            ...
                            <tbody className="bg-gray-700/30 divide-y divide-gray-700/50">
                              {produtosFornecedor.map((produto) => {
                                const garantia = getGarantiaById(produto.garantia_id)
                                return (
                                  <tr key={produto.id} className="hover:bg-gray-600/30 transition-colors">
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap">
                                      {garantia ? (
                                        ...
                                        <span className="text-sm text-gray-500">Sem garantia</span>
                                      )}
                                    </td>
                                  </tr>
                                )
