...
const Sidebar = ({ activePage, setActivePage, isOpen = true, onClose, isMobile = false }: SidebarProps) => {
  ...
  const Icon = ({ children, isActive }: { children: React.ReactNode, isActive: boolean }) => (
    <span className={`flex-shrink-0 w-5 h-5 flex items-center justify-center [&>svg]:w-5 [&>svg]:h-5 ${isActive ? 'text-white' : 'text-gray-400'}`}>
      {children}
    </span>
  )

  const menuItems: { id: Page; label: string; subtitle?: string; icon: React.ReactNode }[] = [
    { id: 'visao-geral' as Page, label: 'Visão Geral', icon: (
    ...
    { id: 'empreendimento' as Page, label: 'Empreendimentos', icon: (
    ...
    { id: 'garantias' as Page, label: 'Garantias', icon: (
    ...
    { id: 'preventivos' as Page, label: 'Preventivas', icon: (
    ...
    { id: 'manutencao-uso' as Page, label: 'Manutenção e Uso', icon: (
    ...
    { id: 'inspecao-laudo' as Page, label: 'Governança Técnica', icon: (
    ...
    { id: 'locais' as Page, label: 'Locais', icon: (
    ...
    { id: 'produtos' as Page, label: 'Produtos', icon: (
    ...
    { id: 'fornecedores' as Page, label: 'Fornecedores', icon: (
    ...
    { id: 'contatos' as Page, label: 'Contatos', subtitle: 'Apoio ao processo', icon: (
    ...
    { id: 'documentos' as Page, label: 'Documentos', icon: (
    ...
    { id: 'perguntas-frequentes' as Page, label: 'Perguntas Frequentes', subtitle: 'Orientação de uso', icon: (
    ...
    { id: 'sobre-nos' as Page, label: 'Sobre Nós', icon: (
    ...
    { id: 'boletim-informativo' as Page, label: 'Boletim Informativo', icon: (
    ...
    { id: 'assistencia-tecnica' as Page, label: 'Assistência Técnica', subtitle: 'Ponto crítico', icon: (
          ...
          {filteredMenuItems.map((item) => (
            <li key={item.id}>
              <button
                ...
                className={`w-full flex items-center justify-start text-left gap-3 pl-4 pr-3 py-3 rounded-lg transition-all duration-200 ${
                  activePage === item.id
